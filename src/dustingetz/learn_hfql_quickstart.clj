...
(tests
  ...
  #_(clojure.pprint/pprint xs) ; todo can it work? ; G yes but it prints a lot.
  xs
  )
...
(comment
  ...
  (hfql/pull (hfql [{:Buckets {* [:Name]}}] (aws/invoke !s3 {:op :ListBuckets})))
  (def s3-index (clojure.datafy/datafy !s3))
  (keys s3-index)
  (hfql/pull (hfql [:api :region {:ops keys}] s3-index)) ; inline fn application at leaf
  )
...
(comment
  ...
  (hfql/pull (hfql {* [type {val [type str]}]} xs)) ; iterate the map's mapentries
  (hfql/pull (hfql {vals {* [type str]}} xs)) ; iterate the map's vals
